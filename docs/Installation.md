...
Download the current `DynaTrade-<version>.jar` file from the official website or release/download page.
...
For the current `0.8.4` line, pay special attention to the `apply:`, `trade.admission:`, `observability:`, and delivery/recovery-related persisted state files. Older configs may not include every newer control yet.
